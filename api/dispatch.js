...
// OJO: aquí NO quemes credenciales en código en prod.
// Para demo, lo dejamos por env o constantes. Yo te lo dejo por ENV:
...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const [key, rows] of groups.entries()) {
      ...
      // 3.1 Claim: RECEIVED -> DISPATCHING (evita doble ejecución)
      ...
      if (claimErr) {
        results.push({ activity_id, contact_list_id, ok: false, step: "claim", error: claimErr.message });
        continue;
      }
      ...
      // 3.2 Armar body EXACTO para Genesys (por cada fila)
      const bodyAll = claimed.map(r => ({
        ...
        data: {
          ...
          status: "NEW" // este status es el de "data" que tú mandas a Genesys
        },
        callable: true
      }));

      // 3.3 (IMPORTANTE) Si son muchos, lo partimos en lotes
    ...
    return res.status(500).json({ ok: false, error: "dispatch_failed", details: String(err?.message || err) });
  }
}